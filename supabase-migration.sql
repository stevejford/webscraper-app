-- Migration SQL to update existing Supabase schema for enhanced session management
...
-- Step 7: Keep existing stored_files table and enhance it
-- (We'll add columns to stored_files in the next step instead of creating a new table)

-- Step 8: Handle existing stored_files table
-- Since stored_files already exists as a table, we'll keep it and add compatibility columns
-- Add new columns to stored_files table to match downloaded_content structure
...
-- Update existing stored_files records to populate new columns
UPDATE stored_files
SET
...
-- Create downloaded_content as a view of stored_files for new code compatibility
...
-- Step 9: Enhanced indexes for performance
...
-- Note: session_id and content_hash indexes likely already exist

-- Step 10: Session lifecycle management functions
...
-- Step 11: Function for validated state transitions
...
-- Step 12: Function for progress checkpointing
...
-- Step 13: RLS is already enabled on stored_files table
-- No need to enable RLS on downloaded_content since it's a view of stored_files

-- Step 14: Ensure RLS policy exists for stored_files (downloaded_content compatibility)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'stored_files' AND policyname = 'Users can view session files'
    ) THEN
...
-- Step 15: Update existing sessions to have proper status
UPDATE scraping_sessions 
SET status = CASE 
...
-- Step 16: Populate progress column for existing sessions
UPDATE scraping_sessions 
